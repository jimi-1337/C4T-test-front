...
const Search = () => {
    ...
    const handleRegistration = (data: any) => {
        ...
        movies.map((result) => {
          if (data.Movie) {
            if (result.title.toLowerCase().includes(data.Movie.toLowerCase())) {
              tmp.push(result.title);
            }
          }
        })
        if(tmp.length == 0)
    ...
    return (
        <>
            ...
            </form>
            {
                ...
                &&
                  <>
                    <div  id="dropdownsearch" data-testid="dropdownsearch"  className="z-10 bg-white divide-y divide-gray-100 rounded shadow w-48 dark:bg-transparent bg-transparent" style={{position: "absolute", margin: "2.5rem 0px 0px 0px", right: '11rem'}}>
